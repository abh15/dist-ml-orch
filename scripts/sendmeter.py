...
onosaddr= "http://10.66.2.142:8181"
...
for i in range(1,4):
    url1= onosaddr+'/onos/v1/meters/of%3A000000000000000'+str(i)
    ...
    for j in range(1,11):
        url2= onosaddr+'/onos/v1/flows/of%3A000000000000000'+str(i)+'?appId=org.onosproject.core'
...
url1= onosaddr+'/onos/v1/meters/of%3A000000000000000a'
data1= '{ "deviceId": "of:000000000000000a", "unit": "KB_PER_SEC", "burst": true, "bands": [ { "type": "DROP", "rate": "100000", "burstSize": "100000" } ] }'
response = requests.post( url1, data=data1, headers=headers, auth=('onos', 'rocks') )
url2= onosaddr+'/onos/v1/flows/of%3A000000000000000a?appId=org.onosproject.core'
